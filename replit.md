...
**Unified Login System Management (August 2025)**: Consolidated multiple department-specific login pages into a streamlined two-tier authentication system. Removed redundant login pages (financial-login, warehouse-login, logistics-login) and their associated routes, maintaining only admin-login and customer-login as the core authentication entry points. This unified approach eliminates user confusion and provides clear separation between administrative and customer access levels. Enhanced useAuth hook to support both admin and customer authentication through intelligent endpoint detection. Added backward compatibility by maintaining both /admin/finance-orders (original) and /admin/financial-workflow (new) routes. Resolved duplicate key issues in i18n translation files and cleaned up unnecessary component imports in App.tsx. System now enforces centralized authentication with proper role-based access control through two distinct portals.
