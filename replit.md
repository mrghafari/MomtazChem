...
**Complete Payment Method Integration and Gateway Routing System (August 2025)**: Resolved all payment routing issues by implementing comprehensive payment method support across both shop and customer order endpoints. Fixed wallet combined payment system to properly route remaining amounts to active payment gateways instead of generic redirect pages. Added complete support for all payment methods: pure wallet payments, wallet+gateway combinations, direct bank payments, and online payments. Enhanced BankGatewayRouter with dynamic gateway detection from database, ensuring system adapts automatically when payment gateways are changed. Fixed TypeScript errors in gateway router and eliminated fallback redirect URLs. System now routes all bank-related payments (wallet_partial, wallet_combined, online_payment, bank) through active Shaparak gateway configuration, providing direct payment URLs instead of intermediate redirect pages.

**Wallet + Bank Hybrid Payment Fix (August 2025)**: Successfully resolved wallet_combined payment method errors that were causing "Unhandled payment method" issues. Fixed payment routing logic to properly convert wallet_combined to wallet_partial when partial wallet usage is intended, enabling proper hybrid payments where specified amount is deducted from wallet and remainder is routed to active bank gateway. Updated TypeScript definitions to support all wallet variations (wallet, wallet_full, wallet_partial, wallet_combined) and eliminated LSP diagnostics errors. System now correctly processes hybrid payments by deducting specified wallet amount and automatically routing remaining amount to active Shaparak payment gateway, generating proper payment URLs for seamless user experience.
...
**Integrated Social Media Management via Content Management System (August 2025)**: Resolved social media links display issue in footer by integrating Content Management system with Footer component. Footer component now fetches social media URLs from content_items table instead of footer_settings, enabling proper management through Content Management interface. Added support for all major platforms including Facebook, Twitter, LinkedIn, Instagram, YouTube, Telegram, WhatsApp, WeChat (with QR code support), and TikTok. Social media links are managed in Content Management section under 'Social Media Links' with 26+ configurable items. Enhanced WeChat integration with hover-activated QR code popup for easy scanning. System provides real-time social media link updates without requiring footer management panel.
...
**Comprehensive Order Number Assignment Fix (August 2025)**: Resolved critical issue where regular orders (non-grace period) were not receiving M[YY][NNNNN] format order numbers after financial approval. Enhanced both financial approval endpoints (/api/finance/orders/:id/approve and /api/finance/orders/:orderId/approve) to automatically assign order numbers using the transaction-safe generateOrderNumberInTransaction() method from OrderManagementStorage. System now ensures ALL orders receive proper sequential numbering (M2511111, M2511112, etc.) regardless of payment method. Both grace period orders and regular orders (online payments, wallet payments) now consistently get order numbers upon financial department approval. Added orderNumber field to API responses for frontend tracking. Fixed inconsistent order numbering that was causing tracking and management issues.
