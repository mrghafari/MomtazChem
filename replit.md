...
```
Changelog:
- July 6, 2025: COMPLETED stock synchronization migration - successfully migrated project from Replit Agent to standard environment with PostgreSQL database, resolved critical stock synchronization issue by implementing unified stock API where admin products endpoint always returns current stock from shop products table (source of truth), created bidirectional stock sync with /api/sync-stock and /api/refresh-stock endpoints, admin and shop sections now show identical stock numbers when orders are placed, project ready for production deployment
