...
**Failed Bank Payment Order Cleanup System (August 2025)**: Implemented comprehensive automatic cleanup system for failed bank payments. System includes: (1) **Immediate Cleanup**: Payment callback endpoint `/api/payment/callback` automatically deletes orders when payment fails/cancels/is abandoned, (2) **Order Number Recovery**: Failed orders are completely removed from system and order numbers are freed for reuse by future customers, (3) **Inventory Restoration**: Products reserved in failed orders are automatically returned to available inventory, (4) **Customer Profile Cleanup**: Failed orders do not appear in customer purchase history, maintaining clean profiles, (5) **Automatic Expired Order Cleanup**: Background service runs every hour to clean up orders older than 24 hours (or 73 hours for bank transfer grace period) that remain unpaid, (6) **Pure Wallet Payment Routing**: Pure wallet payments now bypass finance department and go directly to warehouse with proper order management routing. System ensures no orphaned orders remain in the system and maintains clean, sequential order numbering.
