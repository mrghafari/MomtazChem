...
**Vehicle Selection Algorithm Fixed - Cost-First Approach (August 2025)**: Completely restructured the smart vehicle selection algorithm to prioritize cost efficiency above all other factors. Removed all complex scoring bonuses and penalties that were causing counterintuitive selections (expensive pickup trucks for light loads, cheap buses for heavy loads). Implemented pure cost-based scoring using inverse cost calculation (1000000 / totalCost) ensuring cheapest suitable vehicles are always selected. Added safety-critical restrictions only (motorcycle distance limits, hazardous material requirements). Algorithm now consistently selects most economical option: tanker (50,000 IQD) over bus (105,000 IQD) over pickup (250,000 IQD) over heavy trucks (460,000+ IQD) regardless of load weight, providing logical and cost-effective vehicle recommendations.
...
**Complete Bank Payment Management System (August 2025)**: Implemented comprehensive automatic cleanup and grace period management for bank payments. System includes: (1) **Immediate Cleanup**: Payment callback endpoint `/api/payment/callback` automatically deletes orders when payment fails/cancels/is abandoned, (2) **Order Number Recovery**: Failed orders are completely removed from system and order numbers are freed for reuse by future customers, (3) **Inventory Restoration**: Products reserved in failed orders are automatically returned to available inventory, (4) **Customer Profile Cleanup**: Failed orders do not appear in customer purchase history, maintaining clean profiles, (5) **Grace Period Orders Management**: Orders with uploaded bank receipts are sent to finance department for review via `/api/admin/orders/confirm-grace-period-payment` endpoint, (6) **Payment Rejection System**: Rejected payments are marked with rejection reasons and customers receive detailed email notifications via `sendPaymentRejectionNotification` service - orders are preserved with 'payment_rejected' status instead of deletion, (7) **Additional Documents Request System**: Financial managers can request supplementary documents from customers via `requestAdditionalDocuments` parameter, triggering `sendAdditionalDocumentsRequest` email with specific requirements and instructions, (8) **Customer Re-Upload System**: Customers can upload additional documents via `/api/customers/orders/:orderId/upload-additional-documents` endpoint, automatically returning orders to finance review queue with `pending_additional_documents` status management, (9) **Automatic Expired Order Cleanup**: Background service runs every hour to clean up orders older than 24 hours (or 73 hours for bank transfer grace period) that remain unpaid but only removes orders without uploaded receipts, (10) **Pure Wallet Payment Routing**: Pure wallet payments now bypass finance department and go directly to warehouse with proper order management routing. System ensures no orphaned orders remain while preserving customer order history for rejected payments and enabling seamless document supplementation workflow.
