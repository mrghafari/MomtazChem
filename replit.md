...
**Bulk Purchase UI Enhancement with RTL Support (August 2025)**: Enhanced bulk purchase display with comprehensive right-to-left text alignment for Persian content. Moved "خرید عمده" indicator from quantity controls to align with item total pricing for better visual hierarchy. Added 4cm spacing (mr-16) for optimal positioning. Implemented full RTL support for bulk purchase recommendation section including: right-aligned text direction (dir="rtl"), flex-row-reverse layout for truck icon positioning, text-right alignment for all headings and paragraphs, and margin adjustments (ml-4 instead of mr-4) for proper Persian text flow. This ensures proper Persian language presentation while maintaining functionality and visual appeal.

**Payment Methods System Repair (August 2025)**: Resolved recurring issue where payment methods would disappear from checkout form. Added missing bank_receipt method to payment_method_settings table, bringing total to 4 active payment options: online_payment (priority 4), wallet (priority 3), bank_receipt (priority 2), and bank_transfer_grace (priority 1). Enhanced payment methods rendering in bilingual-purchase-form.tsx to properly handle all dynamic payment methods from admin settings. Added debugging logging to track payment methods loading. System now displays all available payment options consistently without random disappearances.

**Fail-Safe Payment Methods Protection System (August 2025)**: Implemented comprehensive fail-safe protection to ensure checkout form never breaks due to payment methods issues. Added hardcoded FALLBACK_PAYMENT_METHODS with 3 core payment options (online_payment, wallet, bank_transfer_grace) that guarantee checkout functionality regardless of API failures. Enhanced query with retry logic (3 attempts), stale time caching (5 minutes), and robust error handling. System automatically falls back to hardcoded payment methods if API fails, with visual indicator showing fallback status. Removed bank_receipt as standalone payment method (disabled in database) since it's part of bank transfer process, not independent option. This ensures customers can always complete purchases even during API outages or database issues.