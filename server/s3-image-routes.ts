...
export function registerS3ImageRoutes(app: Express) {
  ...
  app.get('/uploads/product-images/:fileName', async (req: Request, res: Response, next: NextFunction) => {
    ...
    try {
      ...
      if (!fileBuffer) {
        console.log(`❌ [S3 PRODUCT IMAGE] File not found in S3: ${req.params.fileName}, serving company logo instead`);
        ...
        try {
          ...
          if (logoBuffer) {
            ...
            res.send(logoBuffer);
            console.log(`✅ [S3 PRODUCT IMAGE] Served company logo as fallback`);
            return;
          }
        } catch (logoError) {
          console.error('❌ [S3 PRODUCT IMAGE] Error serving logo fallback:', logoError);
        }
