...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.post("/api/warehouse/sync-from-kardex", async (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({ success: false, message: "خطا در همگام‌سازی از کاردکس" });
    }
  });
  ...
  app.get("/api/warehouse/products", async (req: Request, res: Response) => {
    try {
      ...
      res.status(500).json({ success: false, message: "خطا در بارگیری محصولات" });
    }
  });
  ...
  app.get("/api/warehouse/items", async (req: Request, res: Response) => {
    try {
      const { productId, category, type, lowStock } = req.query;
      
      let query = db.select({
        id: warehouseItems.id,
        warehouseProductId: warehouseItems.warehouseProductId,
        batchNumber: warehouseItems.batchNumber,
        batchCode: warehouseItems.batchCode,
        currentStock: warehouseItems.currentStock,
        unitPrice: warehouseItems.unitPrice,
        totalValue: warehouseItems.totalValue,
        productionDate: warehouseItems.productionDate,
        expiryDate: warehouseItems.expiryDate,
        supplierName: warehouseItems.supplierName,
        supplierCode: warehouseItems.supplierCode,
        qualityGrade: warehouseItems.qualityGrade,
        ...
        .leftJoin(warehouseProducts, eq(warehouseItems.warehouseProductId, warehouseProducts.id))
      ...
      if (productId) {
        query = query.where(eq(warehouseItems.warehouseProductId, Number(productId)));
      }
      ...
      if (category) {
        filteredItems = filteredItems.filter(item => item.warehouseProduct?.category === category);
      }
      if (type) {
        filteredItems = filteredItems.filter(item => item.warehouseProduct?.productType === type);
      }
      if (lowStock === 'true') {
        filteredItems = filteredItems.filter(item => 
          item.currentStock <= (item.warehouseProduct?.minStockLevel || 0)
        );
      }
      ...
      res.status(500).json({ success: false, message: "خطا در بارگیری بچ‌های کالا" });
    }
  });

  // Get warehouse receipts (ورود به انبار)
  app.get("/api/warehouse/receipts", async (req: Request, res: Response) => {
    try {
      const receipts = await db.select({
        id: warehouseReceipts.id,
        receiptNumber: warehouseReceipts.receiptNumber,
        receiptDate: warehouseReceipts.receiptDate,
        warehouseProductId: warehouseReceipts.warehouseProductId,
        ...
        qualityControl: warehouseReceipts.qualityControl,
        qualityNotes: warehouseReceipts.qualityNotes,
        ...
        updatedAt: warehouseReceipts.updatedAt
      }).from(warehouseReceipts)
        .orderBy(warehouseReceipts.receiptDate);
  ...
  app.get("/api/warehouse/issues", async (req: Request, res: Response) => {
    try {
      const issues = await db.select({
        ...
        vehicleInfo: warehouseIssues.vehicleInfo,
        driverName: warehouseIssues.driverName,
        ...
        createdAt: warehouseIssues.createdAt,
        updatedAt: warehouseIssues.updatedAt
      }).from(warehouseIssues)
        .orderBy(warehouseIssues.issueDate);
  ...
  app.get("/api/warehouse/stats", async (req: Request, res: Response) => {
    try {
      const totalProducts = await db.select({ count: sql`count(*)` }).from(warehouseProducts).where(eq(warehouseProducts.isActive, true));
      const totalItems = await db.select({ count: sql`count(*)` }).from(warehouseItems).where(eq(warehouseItems.isActive, true));
      const totalReceipts = await db.select({ count: sql`count(*)` }).from(warehouseReceipts);
      const totalIssues = await db.select({ count: sql`count(*)` }).from(warehouseIssues);

      res.json({
        totalProducts: totalProducts[0]?.count || 0,
        totalItems: totalItems[0]?.count || 0,
        lowStockItems: 0, // Will be calculated based on minStockLevel
        outOfStockItems: 0, // Items with currentStock = 0
        totalValue: 0, // Sum of all item values
        pendingReceipts: totalReceipts[0]?.count || 0,
        pendingIssues: totalIssues[0]?.count || 0,
        monthlyReceipts: 0,
        monthlyIssues: 0
      });
    } catch (error) {
