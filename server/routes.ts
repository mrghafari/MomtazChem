...
momtazchem.com
  ...
  app.get("/api/admin/security/report", requireAuth, async (req: Request, res: Response) => {
    try {
      const { securityMonitor } = await import("./security-monitor");
  ...
  // Record admin activity manually
  app.post("/api/admin/security/activity", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (!action || !adminId) {
        return res.status(400).json({
          success: false,
          message: "Action and admin ID are required"
        });
      }

      const { securityMonitor } = await import("./security-monitor");
      securityMonitor.recordAdminActivity(adminId, action, details || {});
  ...
  app.post("/api/admin/security/file-upload", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (!filename || !fileSize || !fileType || !userId) {
        return res.status(400).json({
          success: false,
          message: "File details and user ID are required"
        });
      }

      const { securityMonitor } = await import("./security-monitor");
      securityMonitor.recordFileUpload(userId, filename, fileSize, fileType);
      
      res.json({
        success: true,
        message: "File upload activity recorded"
      });
    } catch (error) {
      console.error("Error recording file upload activity:", error);
      res.status(500).json({
        success: false,
        message: "Failed to record file upload activity",
        error: error instanceof Error ? error.message : 'Unknown error'
      });
    }
  });
  ...
  app.post("/api/admin/security/suspicious", requireAuth, async (req: Request, res: Response) => {
    try {
      ...
      if (!query || !ip) {
        return res.status(400).json({
          success: false,
          message: "Query and IP address are required"
        });
      }

      const { securityMonitor } = await import("./security-monitor");
      securityMonitor.recordSuspiciousQuery(query, ip);
