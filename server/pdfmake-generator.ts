...
// Invoice PDF Generation with Batch Info
export async function generateInvoicePDFWithPDFMake(
  customer: any,
  order: any,
  batchInfo: any,
  ...
  return new Promise((resolve, reject) => {
    try {
      ...
      // Prepare order items for table
      const orderTableData = [
        [
          { text: 'محصول', style: 'tableHeader' },
          { text: 'تعداد', style: 'tableHeader' },
          { text: 'قیمت واحد (دینار)', style: 'tableHeader' },
          { text: 'شماره بچ', style: 'tableHeader' },
          { text: 'مجموع (دینار)', style: 'tableHeader' }
        ]
      ];
      ...
      if (order.items && order.items.length > 0) {
        order.items.forEach((item: any) => {
          orderTableData.push([
            { text: item.productName || 'نامشخص', style: 'tableCell' },
            { text: item.quantity?.toString() || '0', style: 'tableCell' },
            { text: `${item.unitPrice || 0}`, style: 'tableCell' },
            { text: item.batchNumber || 'نامشخص', style: 'tableCell' },
            { text: `${(item.quantity * item.unitPrice) || 0}`, style: 'tableCell' }
          ]);
        });
      } else {
        orderTableData.push([
          { text: 'هیچ محصولی یافت نشد', style: 'noData', colSpan: 5 },
          {}, {}, {}, {}
        ]);
      }
      ...
      const docDefinition = {
        ...
        content: [
            ...
            columns: [
              {
                width: '50%',
                stack: [
                  { text: 'شرکت ممتاز شیمی', style: 'companyInfo' },
                  { text: 'Momtaz Chemical Company', style: 'companyInfoEn' },
                  { text: 'آدرس: عراق، اربیل', style: 'companyDetails' },
                  { text: 'Address: Iraq, Erbil', style: 'companyDetailsEn' },
                  { text: 'تلفن: +964 770 999 6771', style: 'companyDetails' }
                ]
              },
              {
                width: '50%',
                stack: [
                  { text: `مشتری: ${customer.name || 'نامشخص'}`, style: 'customerInfo' },
                  { text: `Customer: ${customer.name || 'Unknown'}`, style: 'customerInfoEn' },
                  { text: `ایمیل: ${customer.email || 'نامشخص'}`, style: 'customerDetails' },
                  { text: `تلفن: ${customer.phone || 'نامشخص'}`, style: 'customerDetails' }
                ]
              }
            ],
            margin: [0, 0, 0, 20]
          ...
          // Order details
          {
            text: `شماره سفارش: ${order.customerOrderId || order.id}`,
            style: 'orderInfo',
            margin: [0, 0, 0, 5]
          },
          {
            text: `Order Number: ${order.customerOrderId || order.id}`,
            style: 'orderInfoEn',
            margin: [0, 0, 0, 15]
          },
          ...
          // Total amount
          {
            text: `مجموع کل: ${order.totalAmount || 0} دینار عراقی`,
            style: 'totalAmount',
            alignment: 'right',
            margin: [0, 10, 0, 5]
          },
          {
            text: `Total Amount: ${order.totalAmount || 0} IQD`,
            style: 'totalAmountEn',
            alignment: 'right',
            margin: [0, 0, 0, 20]
          },
