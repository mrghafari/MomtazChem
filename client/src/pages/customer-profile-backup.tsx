...
const CustomerProfile = () => {
  ...
  // Helper function to check if order is temporary/grace period
  const isTemporaryOrder = (order: any) => {
    return order.payment_status === 'payment_grace_period' || 
           order.payment_status === 'grace_period' ||
           order.status === 'temporary' ||
           order.current_status === 'payment_grace_period';
  };

  // Helper function to check if temporary order is still active (not expired)
  const isTemporaryOrderActive = (order: any) => {
    if (!isTemporaryOrder(order)) return false;
    const orderDate = new Date(order.createdAt || order.created_at);
    const now = new Date();
    const hoursDiff = (now.getTime() - orderDate.getTime()) / (1000 * 60 * 60);
    return hoursDiff < 72; // 3 days = 72 hours
  };

  // Helper function to check if order is regular/normal
  const isRegularOrder = (order: any) => {
    return !isTemporaryOrder(order) && 
  ...
  // Get one active regular order (most recent)
  const regularOrders = orders.filter(isRegularOrder);
  const activeRegularOrder = regularOrders.length > 0 ? [regularOrders[0]] : [];

  // Get one active temporary order (most recent that hasn't expired)
  const temporaryOrders = orders.filter((order: any) => 
    isTemporaryOrder(order) && isTemporaryOrderActive(order)
  );
  const activeTemporaryOrder = temporaryOrders.length > 0 ? [temporaryOrders[0]] : [];

  // Combine active orders: one regular + one temporary (max 2 orders)
  const activeOrders = [...activeRegularOrder, ...activeTemporaryOrder];

  // History orders: all other orders except the active ones
  const historyOrders = orders.filter((order: any) => {
    const isActiveRegular = activeRegularOrder.length > 0 && order.id === activeRegularOrder[0].id;
    const isActiveTemporary = activeTemporaryOrder.length > 0 && order.id === activeTemporaryOrder[0].id;
    return !isActiveRegular && !isActiveTemporary;
  });

  // Function to render a single order
  const renderOrder = (order: any) => {
    const isTemp = isTemporaryOrder(order);
    const isTempActive = isTemporaryOrderActive(order);
    
    // Calculate remaining time for temporary orders
    let timeRemaining = '';
    if (isTemp) {
      const orderDate = new Date(order.createdAt || order.created_at);
      const now = new Date();
      const hoursDiff = (now.getTime() - orderDate.getTime()) / (1000 * 60 * 60);
      const hoursLeft = 72 - hoursDiff; // 72 hours = 3 days
      timeRemaining = formatTimeRemaining(hoursLeft);
    }

    return (
      <div key={order.id} className={`border rounded-lg p-4 ${isTemp ? 'border-amber-200 bg-amber-50' : 'border-gray-200 bg-white'}`}>
        <div className="flex justify-between items-start mb-3">
          <div>
            <div className="flex items-center gap-2 mb-1">
              ...
              {isTemp && (
                <Badge className={`${isTempActive ? 'bg-orange-100 text-orange-800' : 'bg-red-100 text-red-800'}`}>
                  <Clock className="w-3 h-3 mr-1" />
                  {isTempActive ? 'سفارش موقت - فعال' : 'سفارش موقت - منقضی'}
                </Badge>
              )}
              {!isTemp && (
          ...
          <div className="text-right">
            ...
            <p className="font-semibold text-lg">{parseFloat(order.totalAmount || order.total_amount || 0).toFixed(2)} {order.currency || 'IQD'}</p>
            <Badge className={getStatusColor(order.current_status || order.status)}>
              {getStatusLabel(order.current_status || order.status)}
            </Badge>
            {/* Show tracking code for completed orders */}
            {((order.status === 'completed' || order.current_status === 'completed') && order.deliveryCode) && (
        ...
        {order.items && order.items.length > 0 && (
          <div>
            ...
            <div className="space-y-2">
              <h5 className="font-medium text-gray-700">محتویات سفارش:</h5>
        ...
        <div className="mt-4 pt-3 border-t flex gap-2 flex-wrap">
          {isTemp && isTempActive && (
            <Button
              size="sm"
              onClick={() => handleActivateGracePeriodOrder(order.orderNumber || order.order_number)}
