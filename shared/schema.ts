...
// =============================================================================
// WAREHOUSE INVENTORY MANAGEMENT SCHEMA
// =============================================================================

// Warehouse inventory table for tracking actual warehouse stock with batch information
export const warehouseInventory = pgTable("warehouse_inventory", {
  ...
  batchType: text("batch_type").notNull().default("new"), // new, existing, none (for non-chemical)
  quantity: integer("quantity").notNull().default(0),
  unitPrice: decimal("unit_price", { precision: 10, scale: 2 }),
  totalValue: decimal("total_value", { precision: 12, scale: 2 }),
  location: text("location"), // Warehouse location/section
  ...
  isNonChemical: boolean("is_non_chemical").default(false),
  notes: text("notes"),
  createdAt: timestamp("created_at").notNull().defaultNow(),
  updatedAt: timestamp("updated_at").notNull().defaultNow(),
  createdBy: text("created_by"),
...
// Warehouse inventory movements for tracking stock changes
export const warehouseInventoryMovements = pgTable("warehouse_inventory_movements", {
  id: serial("id").primaryKey(),
  warehouseInventoryId: integer("warehouse_inventory_id").notNull().references(() => warehouseInventory.id),
  productId: integer("product_id").notNull(),
  productName: text("product_name").notNull(),
  productSku: text("product_sku").notNull(),
  batchNumber: text("batch_number").notNull(),
  movementType: text("movement_type").notNull(), // increase, decrease, transfer, adjustment
  quantityChange: integer("quantity_change").notNull(), // Positive for increase, negative for decrease
  quantityBefore: integer("quantity_before").notNull(),
  quantityAfter: integer("quantity_after").notNull(),
  reason: text("reason").notNull(),
  referenceId: text("reference_id"), // Order ID, transfer ID, etc.
...
export const insertWarehouseInventoryMovementSchema = createInsertSchema(warehouseInventoryMovements).omit({
  id: true,
  createdAt: true,
});

export type InsertWarehouseInventoryMovement = z.infer<typeof insertWarehouseInventoryMovementSchema>;
export type WarehouseInventoryMovement = typeof warehouseInventoryMovements.$inferSelect;
